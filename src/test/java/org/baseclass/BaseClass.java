...
public class BaseClass {
	...
	//select all
	public static void sltall() throws AWTException {
		r=new Robot();
		r.keyPress(KeyEvent.VK_CONTROL);
		r.keyPress(KeyEvent.VK_A);
	}
	//delete by robot
	public static void dlt() throws AWTException {
		r=new Robot();
	...
	//select class
	public static Select slt(WebElement element) {
	...
	public static String readDataFrmExcel(String name, int rowNum,int cellNum) throws IOException {
		File f=new File("C:\\TsngPro1\\excel\\datadriven.xlsx");
	...
	//
	public static String readData(String name, int rowNum, int cellNo) throws IOException {
		File f=new File("C:\\TngDailyWrks\\excel\\datadriven.xlsx");
		FileInputStream fis=new FileInputStream(f);
		Workbook w=new XSSFWorkbook(fis);
		...
		Sheet oldSheet = w.getSheet(name);
		Row oldRow = oldSheet.getRow(rowNum);
		Cell oldCell = oldRow.getCell(cellNo);
		String cell = oldCell.getStringCellValue();
		return cell;
